...
// cabinet toggle takes effect on the very next message.
//
...
func (g *Gateway) allowedToolsForSoul(ctx context.Context, us *UserState) []string {
	if us == nil || us.SoulID == uuid.Nil || us.Registry == nil {
		return nil
	}
	...
	if err := db.SelectContext(ctx, &rows,
		...
		g.logger.Warn("soul tools: query failed, allowing all tools",
			"soul_id", us.SoulID.String(), "error", err)
		return nil
	}
	if len(rows) == 0 {
	...
	var allowed []string
	for _, def := range us.Registry.Definitions() {
		name := def.Name
		if meta[name].Core {
